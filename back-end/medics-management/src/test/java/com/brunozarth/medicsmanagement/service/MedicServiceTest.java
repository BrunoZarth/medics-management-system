...
public class MedicServiceTest {
    ...
    void should_find_by_medical_specialty_and_return_medic_list(){
        // Arrange
        when(medicRepository.findByMedicalSpecialty(any(EMedicalSpecialty.class))).thenReturn(List.of(new Medic(), new Medic()));

        // Act & Assert
        assertThat(medicService.findByMedicalSpecialty(EMedicalSpecialty.ANGIOLOGIA).size()).isEqualTo(2);
        verify(medicRepository, times(1)).findByMedicalSpecialty(any(EMedicalSpecialty.class));
        verifyNoMoreInteractions(medicRepository);
    }
